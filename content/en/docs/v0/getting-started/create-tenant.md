...
-   Complete the previous steps of the tutorial to get 
    a [Cozystack cluster]({{% ref "/docs/v0/getting-started/install-cozystack" %}}) running,
    with storage, networking, and management dashboard configured.
    
-   Make sure you can access the dashboard, as described in the
    [previous step of the tutorial]({{% ref "/docs/v0/getting-started/install-cozystack" %}}).
    
-   If you're using OIDC, users and roles must be configured.
    See the [OIDC guide]({{% ref "/docs/v0/operations/oidc" %}}) for details on how to work with the built-in OIDC server.

During [Kubernetes installation]({{% ref "/docs/v0/getting-started/install-kubernetes" %}}) for Cozystack, 
...
To learn more about tenants, read the [Core Concepts]({{% ref "/docs/v0/guides/concepts#tenant-system" %}}) guide.
...
[OIDC guide]({{% ref "/docs/v0/operations/oidc/enable_oidc#step-4-retrieve-kubeconfig" %}}).
