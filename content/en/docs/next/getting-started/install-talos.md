...
weight: 10
aliases:
  - /docs/next/getting-started/first-deployment
  - /docs/next/getting-started/deploy-cluster
---

## Before you begin

Make sure that you have nodes (bare-metal servers or VMs) that fit the
[hardware requirements]({{% ref "/docs/next/getting-started/requirements" %}}).
...
There are multiple ways to [install Talos Linux for Cozystack]({{% ref "/docs/next/install/talos" %}}), not used here and covered in separate guides.
...
Continue the Cozystack tutorial by [installing and bootstrapping a Kubernetes cluster using Talm]({{% ref "/docs/next/getting-started/install-kubernetes" %}}).
...
-   Learn more about [`boot-to-talos`]({{% ref "/docs/next/install/talos/boot-to-talos#about-the-application" %}}).
