...
weight: 100
aliases:
  - /docs/next/development/development
---
...
4. **Platform chart** (`packages/core/platform`) is deployed as a regular
   ...
   [Package]({{% ref "/docs/next/operations/configuration/platform-package" %}})
...
For the full reconciliation chain (PackageSource → ArtifactGenerator → ExternalArtifact → Package → HelmRelease → Pods), dependency resolution, update and rollback flows, and the cozypkg CLI, see [Key Concepts]({{% ref "/docs/next/guides/concepts" %}}).
...
[variant]({{% ref "/docs/next/operations/configuration/variants" %}}) and
