...
A prerequisite to this step is having [installed a Kubernetes cluster]({{% ref "/docs/next/install/kubernetes" %}}).
...
For a complete guide on deploying Cozystack on generic Kubernetes distributions, see [Deploying Cozystack on Generic Kubernetes]({{% ref "/docs/next/install/kubernetes/generic" %}}).
...
The Platform Package is a `Package` resource that defines the [Cozystack variant]({{% ref "/docs/next/operations/configuration/variants" %}}), [component settings]({{% ref "/docs/next/operations/configuration/components" %}}),
...
refer to the [variants overview and comparison]({{% ref "/docs/next/operations/configuration/variants" %}}).

### 2.2. Fine-tune the Components

You can add some optional components or remove ones that are included by default.
Refer to the [components reference]({{% ref "/docs/next/operations/configuration/components" %}}).
...
-   [Create a user tenant]({{% ref "/docs/next/getting-started/create-tenant" %}}).
