...
For new production deployments, [Talos Linux]({{% ref "/docs/next/guides/talos" %}}) is recommended due to its security and operational benefits.
...
- **Hardware**: See [hardware requirements]({{% ref "/docs/next/install/hardware-requirements" %}})
...
[`networking.kubeovn.MASTER_NODES`]({{% ref "/docs/next/operations/configuration/platform-package" %}})
...
1. [Configure storage with LINSTOR]({{% ref "/docs/next/getting-started/install-cozystack#3-configure-storage" %}})
2. [Set up the root tenant]({{% ref "/docs/next/getting-started/install-cozystack#51-setup-root-tenant-services" %}})
3. [Deploy your first application]({{% ref "/docs/next/applications" %}})
