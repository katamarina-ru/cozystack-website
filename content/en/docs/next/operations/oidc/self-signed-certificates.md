...
- Cozystack cluster with OIDC enabled (see [Enable OIDC Server]({{% ref "/docs/next/operations/oidc/enable_oidc" %}}))
...
- **Authentication fails**: Check that the user exists in Keycloak and has the required group memberships (see [Users and Roles]({{% ref "/docs/next/operations/oidc/users_and_roles" %}})).
