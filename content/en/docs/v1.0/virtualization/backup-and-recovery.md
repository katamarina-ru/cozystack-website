...
weight: 40
aliases:
  - /docs/v1.0/guides/backups
  - /docs/v1.0/kubernetes/backup-and-recovery
---

Cluster backup **strategies** and **BackupClasses** are configured by cluster administrators. If your tenant does not have a BackupClass yet, ask your administrator to follow the [Velero Backup Configuration]({{% ref "/docs/v1.0/operations/services/velero-backup-configuration" %}}) guide to set up storage, strategies, and BackupClasses.
...
{{% alert color="info" %}}
The BackupClass must include a strategy for `VMDisk`. Ask your administrator to add one if it is missing (see [Velero Backup Configuration]({{% ref "/docs/v1.0/operations/services/velero-backup-configuration" %}})).
{{% /alert %}}
