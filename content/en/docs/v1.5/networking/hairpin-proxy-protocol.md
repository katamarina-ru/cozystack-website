...
1. Отключите внедрение PROXY-протокола на вышестоящем LB (предварительное условие для внешнего трафика).
2. Удалите Package CR с помощью `kubectl delete package.cozystack.io cozystack.ouroboros`. Это запускает удаление Helm, которое вызывает pre-delete hook чарта и автоматически вносит изменения в `kube-system/coredns`. Добавление имени в `bundles.disabledPackages` **не** является вторым способом достичь того же результата. `disabledPackages` не удаляет уже установленные компоненты: этот список лишь не даёт платформенному чарту сгенерировать документ Package, а `helm.sh/resource-policy: keep` оставляет активный Package в кластере вместе с принадлежащим ему HelmRelease. ouroboros продолжает работать, pre-delete hook никогда не вызывается, и никакая настройка значений платформы не достигает его.
3. Установите `publishing.proxyProtocol: false` в значениях платформы. Проверка рендеринга теперь проходит (после шага 2 `lookup` для `cozystack.ouroboros` возвращает nil), поэтому `publishing.proxyProtocolAcknowledgeUnclean` остаётся со значением по умолчанию `false`.
