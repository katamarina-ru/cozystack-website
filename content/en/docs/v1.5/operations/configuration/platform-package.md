...
| `publishing.certificates.wildcardSecretName` | `""` | Готовый wildcard TLS Secret, предоставленный оператором. Если задан, платформенные сервисы и ingress/Gateway корневого tenant используют этот уже существующий Secret вместо выпуска отдельных ACME-сертификатов для каждого хоста; через канал values передаётся только ИМЯ, но никогда сам сертификат или приватный ключ. Secret должен уже существовать в namespace публикации (`tenant-root` по умолчанию — namespace, в котором работает корневой ingress controller, выбранный через `publishing.ingressName`), иметь тип `kubernetes.io/tls` и покрывать обслуживаемые хосты (обычно `*.<root-host>` + `<root-host>`). Оставьте пустым, чтобы сохранить выпуск через ACME (см. `solver` / `issuerName` выше). |
