...
Чарт поддерживает два варианта резервного копирования; для каждого релиза выберите один из них.
...
3. Тенант создает `Plan` (расписание cron) или отправляет разовый `BackupJob`,
...
> Secret BucketInfo приложения Cozystack `Bucket` (`bucket-<name>-<user>`,
> единственный ключ `BucketInfo` содержит JSON-объект) **нельзя напрямую
> использовать** как `s3CredentialsSecret`. Либо передайте чарту исходные
...
| `backup.useSystemBucket`                        | Необязательная возможность: при значении true чарт не создает Secret `<release>-backup-s3`, а sidecar-контейнер `clickhouse-backup` считывает параметры подключения к бакету и учетные данные S3 из предоставляемого платформой Secret `cozy-backups-creds`. Для изоляции тенантов друг от друга `S3_PATH` получает значение `<namespace>/<release>`. Используйте вместе с платформенным `cozy-default` BackupClass; тенантам не нужно заполнять поля `s3*` ниже. | `bool`   | `false`                                                |
| `backup.s3Region`                               | УСТАРЕЛО. От конфигурации S3 для отдельных тенантов постепенно отказываются в пользу управляемого платформой `cozy-default` BackupClass. Теперь параметр необязателен, поэтому тенанты с `useSystemBucket: true` могут его не указывать.                                                                                                                                                                     | `string` | `us-east-1`                                            |
...
| `backup.s3PathOverride`                         | УСТАРЕЛО. Префикс ключей объектов внутри прежнего `s3Bucket`; в новом платформенном сценарии с BackupClass область действия автоматически ограничивается пространством имен.                                                                                                                                                                                                                                | `string` | `""`                                                   |
...
| `backup.s3AccessKey`                            | УСТАРЕЛО. Тенанты больше не предоставляют ключи S3; источником достоверных данных служит Secret бакета под управлением платформы. Теперь параметр необязателен, поэтому тенанты с `useSystemBucket: true` могут его не указывать.                                                                                                                                                                               | `string` | `<your-access-key>`                                    |
...
| `backup.s3CredentialsSecret.name`               | Имя Secret в пространстве имен приложения. Пустое значение означает, что чарт создает `<release>-backup-s3` из прежних полей `s3*`.                                                                                                                                                                                                                                                                          | `string` | `""`                                                   |
