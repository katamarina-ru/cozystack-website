---
title: "Управляемый (тенант) Kubernetes"
...
- **`ephemeralStorage` переименован в `diskSize`**: поле `nodeGroups[name].ephemeralStorage` переименовано в `nodeGroups[name].diskSize`, чтобы точнее отражать назначение поля: persistent disk для данных kubelet и containerd. Fallback для обратной совместимости отсутствует; пользователи ДОЛЖНЫ обновить конфигурации и использовать `diskSize` вместо `ephemeralStorage`. Если `ephemeralStorage` все еще присутствует в values, рендеринг Helm template завершится ошибкой с указанием использовать `diskSize`. При прямом обновлении CRD в обход Helm нераспознанное поле тихо отбрасывается, а storage kubelet возвращается к значению по умолчанию 20Gi. Существующие VM будут автоматически обновляться по rolling update через CAPI после применения новых значений. Состояние сохраняется при перезагрузках той же VM (restart virt-launcher, guest reboot, отказ ноды); замена VM через CAPI (например, изменение поля nodeGroup или remediation через MachineHealthCheck) создает новый PVC.
...
| Имя                                             | Описание                                                                                                                                                                           | Тип                 | Значение    |
| ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ----------- |
| `nodeGroups`                                    | Конфигурации worker nodes.                                                                                                                                                     | `map[string]object` | `{...}`     |
| `nodeGroups[name].minReplicas`                  | Минимальное количество реплик.                                                                                                                                                     | `int`               | `0`         |
| `nodeGroups[name].maxReplicas`                  | Максимальное количество реплик.                                                                                                                                                    | `int`               | `10`        |
| `nodeGroups[name].instanceType`                 | Тип инстанса виртуальной машины.                                                                                                                                                  | `string`            | `u1.medium` |
| `nodeGroups[name].diskSize`                     | Размер диска для данных kubelet и containerd.                                                                                                                           | `quantity`          | `20Gi`      |
| `nodeGroups[name].storageClass`                 | StorageClass для диска worker node. Если пусто, используется default StorageClass management-кластера (с annotation storageclass.kubernetes.io/is-default-class: true). | `string`            | `""`        |
| `nodeGroups[name].roles`                        | Список ролей ноды.                                                                                                                                                                 | `[]string`          | `[]`        |
| `nodeGroups[name].resources`                    | CPU и memory resources для каждой worker ноды.                                                                                                                                     | `object`            | `{}`        |
| `nodeGroups[name].resources.cpu`                | Доступный CPU.                                                                                                                                                                     | `quantity`          | `""`        |
| `nodeGroups[name].resources.memory`             | Доступная память (RAM).                                                                                                                                                            | `quantity`          | `""`        |
| `nodeGroups[name].gpus`                         | Список GPU для подключения (драйвер NVIDIA требует минимум 4 GiB RAM).                                                                                                             | `[]object`          | `[]`        |
| `nodeGroups[name].gpus[i].name`                 | Имя GPU, например "nvidia.com/AD102GL_L40S".                                                                                                                                       | `string`            | `""`        |
| `nodeGroups[name].kubelet`                      | Резервации ресурсов kubelet для этой node group.                                                                                                                                   | `object`            | `{}`        |
...
| `nodeGroups[name].kubelet.evictionHardMemory`   | Жёсткий порог вытеснения по памяти (абсолютное значение вроде 200Mi или процент вроде 7%).                                                                                       | `string`            | `7%`        |
| `nodeGroups[name].kubelet.evictionSoftMemory`   | Мягкий порог вытеснения по памяти (абсолютное значение вроде 1Gi или процент вроде 10%).                                                                                         | `string`            | `10%`       |
| `version`                                       | Версия Kubernetes major.minor для развертывания.                                                                                                                                   | `string`            | `v1.35`     |
| `host`                                          | Внешний hostname Kubernetes-кластера. Если пусто, используется `<cluster-name>.<tenant-host>`.                                                                                     | `string`            | `""`        |


### Расшиерения кластера
