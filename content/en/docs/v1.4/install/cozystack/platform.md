...
Предварительное условие для этого шага — [установленный кластер Kubernetes]({{% ref "/docs/v1.4/install/kubernetes" %}}).

Если вы устанавливаете Cozystack впервые, рекомендуем начать с [руководства по Cozystack]({{% ref "/docs/v1.4/getting-started" %}}).
...
Полное руководство по развертыванию Cozystack на generic-дистрибутивах Kubernetes см. в разделе [Развертывание Cozystack на Generic Kubernetes]({{% ref "/docs/v1.4/install/kubernetes/generic" %}}).

## 2. Описание и применение Platform Package

После запуска operator следующий шаг — описать Platform Package и применить его.
Platform Package — это ресурс `Package`, который определяет [variant Cozystack]({{% ref "/docs/v1.4/operations/configuration/variants" %}}), [настройки компонентов]({{% ref "/docs/v1.4/operations/configuration/components" %}}),
...
см. [обзор и сравнение variants]({{% ref "/docs/v1.4/operations/configuration/variants" %}}).

### 2.2. Тонкая настройка компонентов

Можно добавить дополнительные компоненты или убрать компоненты, включенные по умолчанию.
См. [справочник components]({{% ref "/docs/v1.4/operations/configuration/components" %}}).
...
См. раздел [установка у конкретных провайдеров]({{% ref "/docs/v1.4/install/providers" %}}).
...
{{% alert color="info" %}}
Cozystack по умолчанию собирает анонимную статистику использования. Подробнее о том, какие данные собираются и как отказаться от сбора, см. в [документации по Telemetry]({{% ref "/docs/v1.4/operations/configuration/telemetry" %}}).
{{% /alert %}}
...
- [Настройте OIDC]({{% ref "/docs/v1.4/operations/oidc/" %}}).
- [Создайте user tenant]({{% ref "/docs/v1.4/getting-started/create-tenant" %}}).
nant" %}}).
