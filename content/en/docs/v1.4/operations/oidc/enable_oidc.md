...
{{% alert color="info" %}}
**Опционально**: если нужно, чтобы dashboard обращался к Keycloak через внутреннюю сеть кластера вместо external ingress, задайте `keycloakInternalUrl`. Это полезно в окружениях с self-signed certificates или ограниченным внешним доступом. Подробности см. в [Self-Signed Certificates]({{% ref "/docs/v1.4/operations/oidc/self-signed-certificates" %}}).
{{% /alert %}}
