...
weight: 40
aliases:
  - /docs/v1.4/guides/backups
  - /docs/v1.4/kubernetes/backup-and-recovery
---
...
{{% alert color="info" %}}
This guide covers VM backups (HelmRelease + CRs + PVC snapshots bundled by Velero). For data-only backups of managed databases (Postgres, MariaDB, ClickHouse, FoundationDB), see [Application Backup and Recovery]({{% ref "/docs/v1.4/applications/backup-and-recovery" %}}).
{{% /alert %}}
...
{{% alert color="info" %}}
The BackupClass must include a strategy for `VMDisk`. Ask your administrator to add one if it is missing (see [Velero Backup Configuration]({{% ref "/docs/v1.4/operations/services/velero-backup-configuration" %}})).
{{% /alert %}}
