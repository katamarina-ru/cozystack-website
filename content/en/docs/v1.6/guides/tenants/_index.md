...
Администраторы и пользователи Cozystack создают tenants с помощью [приложения Tenant]({{% ref "/docs/v1.6/applications/tenant" %}})
...
В tenants могут быть развернуты [cluster services]({{% ref "/docs/v1.6/operations/services" %}}).
...
- [Tenant Kubernetes cluster]({{% ref "/docs/v1.6/kubernetes" %}}) и
  [Postgres database]({{% ref "/docs/v1.6/applications/postgres" %}}) в tenant `bar`.
...
См. [Tenant `isolated` flag removed]({{% ref "/docs/v1.6/operations/upgrades#tenant-isolated-flag-removed" %}})
...
См. справочник по приложению, реализующему управление tenants: [`tenant`]({{% ref "/docs/v1.6/applications/tenant#parameters" %}})
