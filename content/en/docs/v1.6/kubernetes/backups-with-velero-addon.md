...
The `velero` addon of the [Managed Kubernetes]({{% ref "/docs/v1.6/kubernetes" %}}) application installs [Velero](https://velero.io/) inside a tenant Kubernetes cluster. Combined with a tenant [Bucket]({{% ref "/docs/v1.6/operations/services/object-storage/buckets" %}}), it lets tenant users back up workloads to S3 and restore them later.
...
For the platform-level Velero used by cluster administrators to back up `VMInstance`/`VMDisk` resources from the management cluster, see [Backup Classes]({{% ref "/docs/v1.6/operations/services/backup-classes" %}}).
{{% /alert %}}
...
- SeaweedFS is enabled on your tenant (`spec.seaweedfs: true`). See [Object Storage]({{% ref "/docs/v1.6/operations/services/object-storage" %}}).
