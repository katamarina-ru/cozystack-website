...
Инструкции для администраторов приведены в руководстве [Backup Classes]({{% ref "/docs/v1.6/operations/services/backup-classes" %}}).
{{% /alert %}}
...
Сведения о резервных копиях, включающих Helm-релиз приложения, CR и снимки PVC (для VMInstance / VMDisk), см. в разделе [Резервное копирование и восстановление (ВМ)]({{% ref "/docs/v1.6/virtualization/backup-and-recovery" %}}).
{{% /alert %}}
...
Если `status.message` не позволяет точно определить причину сбоя, сообщите администратору имя `BackupJob`. Администратор проверит созданный драйвером CR оператора (см. раздел [Backup Classes]({{% ref "/docs/v1.6/operations/services/backup-classes" %}}) в руководстве для администраторов).
...
If those do not explain the failure, the next layer of diagnostics lives on the operator-native CR the driver created (`cnpg.io/Backup`, `k8s.mariadb.com/Backup`, `etcd-operator.cozystack.io/EtcdSnapshot`, or the ClickHouse strategy `Pod` logs). These resources are not reachable under the tenant kubeconfig — hand the `BackupJob` name to your administrator and they will follow [Backup Classes]({{% ref "/docs/v1.6/operations/services/backup-classes" %}}).

## См. также

- [Backup Classes]({{% ref "/docs/v1.6/operations/services/backup-classes" %}}): сведения о том, как администраторы определяют стратегии и ресурсы `BackupClass` для баз данных и ВМ.
- [Резервное копирование и восстановление (ВМ)]({{% ref "/docs/v1.6/virtualization/backup-and-recovery" %}}): аналогичное руководство по резервному копированию VMInstance / VMDisk (HelmRelease + CR + снимки PVC).
