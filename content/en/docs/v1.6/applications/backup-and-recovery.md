...
Admins follow the [Backup Classes]({{% ref "/docs/v1.6/operations/services/backup-classes" %}}) guide.
{{% /alert %}}
...
For backups that include the application's Helm release, CRs, and PVC snapshots (used for VMInstance / VMDisk), see [Backup and Recovery (VMs)]({{% ref "/docs/v1.6/virtualization/backup-and-recovery" %}}).
{{% /alert %}}
...
If `status.message` does not pinpoint the failure, hand the `BackupJob` name to your administrator and they will inspect the operator-native CR the driver created (see [Backup Classes]({{% ref "/docs/v1.6/operations/services/backup-classes" %}}) in the admin guide).
...
- [Backup Classes]({{% ref "/docs/v1.6/operations/services/backup-classes" %}}) — how administrators define strategies and `BackupClass` resources for databases and VMs.
- [Backup and Recovery (VMs)]({{% ref "/docs/v1.6/virtualization/backup-and-recovery" %}}) — the parallel guide for VMInstance / VMDisk backups (HelmRelease + CRs + PVC snapshots).
