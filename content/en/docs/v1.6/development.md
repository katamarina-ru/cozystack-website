...
For the full reconciliation chain (PackageSource → ArtifactGenerator → ExternalArtifact → Package → HelmRelease → Pods), dependency resolution, update and rollback flows, and the cozypkg CLI, see [Key Concepts]({{% ref "/docs/v1.6/guides/concepts" %}}).
...
[variant]({{% ref "/docs/v1.6/operations/configuration/variants" %}}) and
