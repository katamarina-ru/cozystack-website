...
If you want GPUs in **containers** rather than in VMs, you do not need this recovery at all — use the [`container` variant](/docs/v1.6/operations/gpu-container-workloads/), which is designed for exactly this host shape and keeps the host driver in place.
...
- **Use the [`container` variant](/docs/v1.6/operations/gpu-container-workloads/)** if the workloads can be containers rather than VMs. It targets the apt-installed-driver host shape and exposes GPUs to pods without unbinding the host driver, so no purge is needed.
