...
| `publishing.proxyProtocol` | `false` | Включает PROXY-протокол на хост-инстансе ingress-nginx и автоматически развёртывает [ouroboros]({{% ref "/docs/v1.6/networking/hairpin-proxy-protocol" %}}) для устранения возникающей проблемы hairpin-NAT. Вышестоящий L4-балансировщик перед ingress-nginx должен уже внедрять заголовки PROXY-v1 до включения этого флага; см. связанную страницу для рецептов проверки и пути отключения. |
