...
weight: 5
aliases:
  - /docs/v1.6/getting-started/hardware-requirements
  - /docs/v1.6/talos/hardware-requirements
---

Cozystack utilizes [Talos Linux]({{% ref "/docs/v1.6/guides/talos" %}}), a minimalistic Linux distribution designed solely to run Kubernetes.
...
- **Secondary Disk**: Dedicated to workload data and can be increased based on workload requirements.
  ...
  [Deploy Cozystack tutorial]({{% ref "/docs/v1.6/getting-started/install-cozystack#3-configure-storage" %}})

**Using a single disk**

It's possible to use a single disk with space allocated for user storage.
See [How to install Talos on a single-disk machine]({{% ref "/docs/v1.6/install/how-to/single-disk" %}})
...
You can build a [distributed cluster]({{% ref "/docs/v1.6/operations/stretched/" %}}) with Cozystack.
...
For detailed recommendations on system resource allocation (CPU and memory) per node, based on cluster scale and number of tenants, refer to [System Resource Planning Recommendations]({{% ref "/docs/v1.6/install/resource-planning" %}}).
