...
weight: 10
aliases:
  - /docs/v1.6/talos/installation
  - /docs/v1.6/talos/install
  - /docs/v1.6/operations/talos/installation
...
To plan the installation, see the [hardware requirements]({{% ref "/docs/v1.6/install/hardware-requirements" %}}).
...
-   **Recommended:** [Boot to Talos Linux from another Linux OS using `boot-to-talos`]({{% ref "/docs/v1.6/install/talos/boot-to-talos" %}}) —
...
-   [Install using temporary DHCP and PXE servers running in Docker containers]({{% ref "/docs/v1.6/install/talos/pxe" %}}) — 
    requires an extra management machine, but allows for installing on multiple hosts at once.
-   [Install using ISO image]({{% ref "/docs/v1.6/install/talos/iso" %}}) — optimal for systems which can automate ISO installation.

## Further Steps

-   After installing Talos Linux, you will have a number of nodes ready for the next step, which is to
    [install and bootstrap a Kubernetes cluster]({{% ref "/docs/v1.6/install/kubernetes" %}}).
    
-   Read the [Talos Linux overview]({{% ref "/docs/v1.6/guides/talos" %}}) to learn why Talos Linux is the optimal OS choice for Cozystack
