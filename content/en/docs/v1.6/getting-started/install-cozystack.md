...
см. [руководство BYOP (Build Your Own Platform)]({{% ref "/docs/v1.6/install/cozystack/kubernetes-distribution" %}}).
{{% /alert %}}
...
-   `spec.variant: "isp-full"` означает, что используется наиболее полный набор компонентов Cozystack.
    Подробнее о вариантах см. в [справочнике по вариантам Cozystack]({{% ref "/docs/v1.6/operations/configuration/variants" %}}).
...
Подробнее об этом файле конфигурации см. в [справочнике Platform Package]({{% ref "/docs/v1.6/operations/configuration/platform-package" %}}).

{{% alert color="info" %}}
По умолчанию Cozystack собирает анонимную статистику использования. Подробнее о том, какие данные собираются и как отказаться от этой функции, см. в [документации по телеметрии]({{% ref "/docs/v1.6/operations/configuration/telemetry" %}}).
{{% /alert %}}
...
-   Для ВМ и выделенных серверов у облачных провайдеров выбирайте настройку через публичные IP.
    ...
    Загляните в раздел [provider-specific installation]({{% ref "/docs/v1.6/install/providers" %}}).
