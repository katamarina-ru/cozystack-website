...
Set `gateway.enabled: true` on the `cozystack.cozystack-platform` Package. See the [Platform Package reference]({{% ref "/docs/v1.6/operations/configuration/platform-package" %}}) for the full `gateway.*` and `publishing.certificates.dns01.*` value tables.
...
Set [`publishing.certificates.wildcardSecretName`]({{% ref "/docs/v1.6/operations/configuration/platform-package" %}}) and the tenant leaves ACME entirely: the `TenantGateway` points its listeners at that pre-existing Secret, and the controller mints no `Issuer` and no `Certificate`. The solver, DNS-01 provider and issuer settings are ignored on this path. Listener shape matches DNS-01, so it clears the 64-listener cap the same way.
