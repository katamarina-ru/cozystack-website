...
Оба вышеперечисленных режима являются режимами ACME. Установка [`publishing.certificates.wildcardSecretName`]({{% ref "/docs/v1.6/operations/configuration/platform-package" %}}) выбирает третий режим, **existingSecret**: `TenantGateway` напрямую ссылается на Secret, предоставленный оператором, и контроллер не создаёт для тенанта отдельный `Issuer` и `Certificate` - solver, провайдер DNS-01 и настройки issuer пропускаются на этом пути, а любой `Issuer` или `Certificate`, которым он ранее владел, удаляется сборщиком мусора. Приватный ключ учётной записи ACME (`<tgw>-acme-account`) НЕ удаляется при переключении режима - удаляется только `Issuer`, ссылающийся на него. Он очищается post-delete хуком при удалении релиза gateway тенанта, а не при изменении режима. Общекластерные `ClusterIssuer`ы `letsencrypt-prod` / `letsencrypt-stage` не затрагиваются: они формируются из `publishing.certificates.solver` независимо от этого, и всё равно проверяют настройки своего провайдера DNS-01 во время рендеринга. Форма listener соответствует DNS-01: один listener `*.<apex>`, один listener `<apex>` и один listener `*.<child-apex>` для каждого наследующего child, все они указывают на этот единственный Secret.
...
2. Дождитесь Gateway, а в режимах сертификатов ACME - и Certificate. Gateway всегда носит имя `cozystack`:
   ...
   Не ориентируйтесь на `ResolvedRefs` у "сырого" `Gateway`. На этой платформе `cozystack-controller` перечисляет и `HTTPRoute`, и `TLSRoute` в `allowedRoutes.kinds` на каждом listener'е порта 443 - это обходной путь, не позволяющий Cilium схлопывать их вместе, - и Cilium в ответ выставляет постоянный `ResolvedRefs=False/InvalidRouteKinds` на каждом из них. Это косметическая проблема, не влияющая ни на трафик, ни на готовность (readiness), но из-за неё `ResolvedRefs` показывает `False` на абсолютно здоровом кластере.
...
- **Режим сертификата `existingSecret` предназначен только для root-тенанта, и это ничем не обеспечивается принудительно.** Имя Secret передаётся через канал cluster values всем тенантам, поэтому включение этого режима также переключает дочерние объекты, владеющие gateway, на этот режим (им нужно, чтобы Secret был скопирован в их собственный namespace), а на пути ingress по умолчанию оставляет дочерний объект с собственным ingress-контроллером вообще без сертификата - ingress-nginx отдаёт свой встроенный самоподписанный сертификат. Этот последний случай - открытый баг, [cozystack/cozystack#3296](https://github.com/cozystack/cozystack/issues/3296).
- **Общий IP балансировщика (LB) для нескольких тенантов.** Несколько владеющих тенантов не могут делить один LB IP на текущей версии Cilium: каждый владеющий тенант Gateway занимает `443/TCP`, а `lbipam.cilium.io/sharing-key` не активен при конфликте портов ([cilium#21270](https://github.com/cilium/cilium/issues/21270), [cilium#42756](https://github.com/cilium/cilium/issues/42756)). Поэтому каждому владеющему Gateway нужен собственный LB IP от аллокатора, управляемого администратором, до тех пор, пока Cilium не выпустит ListenerSet. В рамках одного Gateway наследование (родитель + все наследующие дочерние объекты делят один IP) уже работает сегодня.
- **TLSRoute v1alpha2.** Gateway API v1.5 поставляет TLSRoute на версии `v1alpha2`. Выше по течению он перейдёт на `v1`; Cozystack последует за этим переименованием, когда оно произойдёт.
- **Wildcard-сертификаты DNS-01 требуют доступа к DNS-провайдеру для каждого уровня apex.** Когда глубоко вложенное дерево тенантов (например, `tenant-root` → `alice` → `alice-bob`) наследует режим DNS-01 через root, SAN родителя `*.alice.example.org` требует, чтобы ACME-проверка родителя записала TXT-запись под `_acme-challenge.alice.example.org`. Если оператор не делегировал эту подзону аккаунту DNS-провайдера родителя, выпуск сертификата для apex внука зависает. Режим HTTP-01 этим не затронут.
- **Поддерживаемые ACME-эмитенты.** `publishing.certificates.issuerName` должен быть либо `letsencrypt-prod`, либо `letsencrypt-stage` (контроллер сопоставляет их с URL-адресами серверов ACME). Чтобы поддержать другого ACME-провайдера, нужно расширить рендерер контроллера дополнительной веткой.
- **Принудительное применение `tenant.spec.host`.** Тенант не может задать собственный host (это блокируется во время выполнения), но администратор кластера, неправильно настроивший это, может привести к тому, что тенант публикует хостнейм, которым он не владеет. В режимах сертификатов ACME проверка ACME завершится ошибкой (нет контроля над DNS), поэтому сертификат не выпускается и захвата не происходит - хотя диагностика останавливается на "Certificate stuck in Pending". В режиме `existingSecret` этой защитной сетки нет: ничего не выпускается, поэтому ничто не подтверждает контроль над доменом, Gateway переходит в статус `Programmed`, и оператору отдаётся Secret с теми SAN-ами, которые в нём оказались.
- **Функции приложений выше по течению (upstream).** Некоторые функции уровня чарта в harbor / bucket всё ещё зависят от аннотаций ingress-nginx выше по течению. Cozystack отслеживает это как upstream PR-ы; это остаётся причиной, по которой некоторые operations-команды ещё некоторое время будут держать ingress-nginx рядом с Gateway API.
- **Namespace cert-manager зашит в код (hardcoded)** для ACME HTTP-01. Белый список `allowedRoutes` слушателя порта 80 явно указывает `cozy-cert-manager`. Операторы, запускающие cert-manager в non-default namespace, сегодня не могут использовать HTTP-01 с Gateway API - ACME challenge HTTPRoute будет отклонён без очевидной диагностики. Режим DNS-01 этим не затронут (в кластере не участвует HTTPRoute для проверки).
