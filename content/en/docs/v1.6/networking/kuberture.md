...
- [Включение и отключение компонентов]({{% ref "/docs/v1.6/operations/configuration/components#enabling-and-disabling-components" %}}) - используемый здесь механизм `bundles.enabledPackages` / `bundles.disabledPackages`.
- [Справочник Platform Package]({{% ref "/docs/v1.6/operations/configuration/platform-package" %}}) - справка по полю `bundles.enabledPackages`.
